...
| | |
|---|---|
| ホスト | Docker と Compose **v2** が動く Linux / macOS / WSL2 (`docker-compose` では動きません)。ポート 8000 が空いていること (`0.0.0.0` に開きます。Bearer キー必須) |
...
監視 → **ログの回収** → セッション → **サーバへの問い合わせ** → トンネル の順に畳みます。
...
止める前に Colab 側の `/content/logs/{setup,api,comfyui}.log` を
`works/.rescue/<セッション>/logs/` へ持ち帰ります。**ランタイムを止めると `/content` ごと
消える**ので、その回に分かったことを残してから畳みます。急ぐときは `--no-logs` で飛ばせます。
...
落ちた理由は落ちたセッションの中にあります。**止める前に読んでください。**
...
監視も30秒ごとに同じ口 (`colab exec`) を使うので、重なると返りが遅くなります。
返らなかったときはそう出るので、少し置いてからもう一度叩いてください。
...
コードから頼むときは HTTP です。`tunnel` サービスが 8000 をホストへ publish しているので、
**呼ぶ側はこちらの compose の中身を知らずに、ホストの 8000 番だけを見ます。**

```bash
# 呼ぶ側の .env (ホストで動くプロセスから)
COLAB_ENDPOINT=http://127.0.0.1:8000
COLAB_API_KEY=...
```

呼ぶ側がコンテナの中にいるときは、ホスト経由で届きます。
...
services:
  app:
    extra_hosts:
      - "host.docker.internal:host-gateway"   # Docker Desktop 以外では要ります
    environment:
      COLAB_ENDPOINT: http://host.docker.internal:8000
```

以前は共有ネットワーク `comfy-net` に相乗りして `http://tunnel:8000` を解決させていましたが、
その形は**こちらの compose のサービス名とネットワーク名に呼ぶ側が依存する**ことでもありました。
ホスト経由なら、こちらの構成が変わっても呼ぶ側は 8000 番だけを見ます。乗るプロジェクトが
無くなったので、宣言ごと外しました。

8000 は `0.0.0.0` に publish しています。**Bearer キーが要る口**で、キーは 32バイトの乱数を
SHA-256 で持ちます。無認証で答えるのは `/health` だけです。**8188 (ComfyUI) は publish しません。**
...
compose が publish するのは 8000 の1本だけです。8000 は `0.0.0.0` に出していて、コンテナから頼む側 (`host.docker.internal:8000`) の通り道になります。Bearer キーが
要る口で、無認証で答えるのは `/health` だけです。**publish のホスト IP は省かないで
ください。** 省くと 0.0.0.0 になるので、広い側を選ぶときも書いておきます。
