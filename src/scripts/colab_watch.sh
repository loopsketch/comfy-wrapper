#!/usr/bin/env bash
# ランタイムの監視。中身は colab_keepalive_watch.py。
#
#   src/scripts/colab_watch.sh [セッション名] [上限分]   監視を始める
#   src/scripts/colab_watch.sh --status [セッション名]   状態を見る
#   src/scripts/colab_watch.sh --stop                    監視を止める
#
...
case "${1:-}" in
  ...
  *)
    ...
    echo "監視を始めた (上限 ${MAX}分で自動停止 / ログ: .colab/keepalive-watch.log)"
    ;;
esac
