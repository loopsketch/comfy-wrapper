...
手元から API を叩くスクリプトは複数あるが、経路は1本しかない。以前は呼び出しごとに
別々に HTTP を叩いていて、トンネル落ちの整形が片方にしか無く、もう片方では
プロキシの HTML 片がそのまま画面に出ていた。同じ経路の面倒は1か所で見る。
...
def health(endpoint: str, timeout: float = 30.0) -> dict:
    ...
    生成を投げる前の疎通確認はここに寄せる。以前は colab_run.sh がインラインの
    python で同じことを書いていて、こちらの health() は誰も呼んでいなかった。
    """
    try:
